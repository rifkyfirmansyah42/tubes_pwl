@extends('layouts.app_modern', ['title' => 'Edit Pendaftaran'])

@section('content')
<div class="card">
    <div class="card-header">
        Form Edit Pendaftaran Pasien
    </div>
    <div class="card-body">
        <form action="/daftar/{{ $daftar->id }}" method="POST">
            ...
            <div class="form-group mt-3">
                <label for="pasien_id">Nama Pasien | <a href="/pasien/create" target='_blank'>Pasien Baru</a>
                </label>
                <select name="pasien_id" class="form-control select2">
                    <option value="">-- Pilih Pasien --</option>
                    @foreach ($listPasien as $item)
                        <option value="{{ $item->id }}" @selected(old('pasien_id', $daftar->pasien_id) == $item->id)>
            ...
            <div class="form-group mt-3">
                ...
                <select name="poli" class="form-control">
                    ...
                    @foreach ($listPoli as $key => $val)
                        <option value="{{ $key }}" @selected(old('poli', $daftar->poli) == $key)>{{ $val }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group mt-3">
                ...
                <textarea name="keluhan" class="form-control">{{ old('keluhan', $daftar->keluhan) }}</textarea>
